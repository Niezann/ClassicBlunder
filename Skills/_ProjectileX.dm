					...
					if(istype(a, /obj/Skills/Projectile/_Projectile))
						...
						if(src.Striking)
							...
							if(src.DamageMult>=0.4)
								KenShockwave(a, Size=max((src.DamageMult+src.Knockback+src.Owner.Intimidation/50)*max(2*(!src.Owner.HasNullTarget() ? src.Owner.GetGodKi()+src.Owner.GetMaouKi() : 0),1)*GoCrand(0.04,0.4),0.2),PixelX=src.VariationX,PixelY=src.VariationY)
						if(src.Slashing)
